import burgerReducer, {
  ...
  describe('тест burgerReducer', () => {
    it('добавляем ингредиент', () => {
      ...
      expect(state.constructorItems.ingredients).toEqual([mainIngredient]);
    });
    ...
    it('проверяем, что заменяется булка, если уже была выбрана какая-то булка', () => {
      ...
      expect(state.constructorItems.ingredients).toEqual([newBun]);
    });
    ...
    it('удаляем ингредиент из конструктора', () => {
      ...
      expect(state.constructorItems.ingredients).toEqual([bun, sauce]);
    });
    ...
    it('перемещает ингредиент в конструкторе вверх', () => {
      ...
      const action = moveCartItem({ itemId: sauce.id, direction: 'up' });
      const state = burgerReducer(stateWithIngredients, action);
  
      expect(state.constructorItems.ingredients).toEqual([bun, sauce, mainIngredient]);
    });
    ...
    it('перемещает ингредиент в конструкторе вниз', () => {
      ...
      const action = moveCartItem({ itemId: mainIngredient.id, direction: 'down'});
      const state = burgerReducer(stateWithIngredients, action);
      expect(state.constructorItems.ingredients).toEqual([bun, sauce, mainIngredient]);
    });
  });
